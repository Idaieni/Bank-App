...
void InterestPlan::CalculateInterest() {
	...
	currentMonth = 1;
	//1 is picked for better consistency in equations
	totalMonths = numYears * 12;
	total = startingAmount;
	//total is initially the starting amount
	interestAmount = 0;
	...
	while(currentMonth <= totalMonths) {
		...
		//first we add totalto the deposit
		interestAmount = total * monthlyInterest;
		//calculate interest gained
		interestList.push_back(interestAmount);
		//save interest at the endof vector list, to be accessed later
		total += interestAmount;
		//add interest tototal for a new total
		sumList.push_back(total);
		//save total to be accessed later
		currentMonth++;
